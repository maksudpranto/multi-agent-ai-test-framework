...
def _materialize_prompt(
    *, signature: str, params: list[dict] | None, suite_cases: list[dict],
    ...
    return (
        ...
        f"{cases_json}\n\n"
        "Task: produce the concrete list of argument tuples this suite implies — "
        "one tuple per distinct scenario the suite tries to cover (happy path, "
        "boundary values, invalid/negative inputs). Cover the boundaries the "
        "suite hints at.\n\n"
